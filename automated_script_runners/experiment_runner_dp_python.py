...
algo = "dp"
CSV_FILENAME = "py_" + algo + "_results.csv"
...
def list_files_in_instances():
    ...
    for root, _, files in os.walk(instances_folder):
        for file in files:
            # Only proceed if 'no_conflict' is in the filename
            if "no_conflict" not in file:
                print(f"Skipping {file} (no 'no_conflict' in filename)")
                continue

            file_path = os.path.join(root, file)
            execute_python_solver(file_path)
...
def execute_python_solver(instance_path):
    command = ["python", "src/DynamicProgrammingKP01.py"]
